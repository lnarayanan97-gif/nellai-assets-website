...
$('#search')?.addEventListener('click',()=>{
 const t=$('#stype').value,a=$('#sarea').value.trim().toLowerCase();let n=0;
 cards.forEach(c=>{const ok=(t==='all'||c.dataset.type===t)&&(!a||c.dataset.area.includes(a));c.style.display=ok?'':'none';if(ok)n++});
 $('#none').hidden=n!==0;$('#searchNote').textContent=`${n} ${n===1?'category':'categories'} match your search.`;
...
function calc(){
 const p=Math.max(0,+$('#price').value||0),d=Math.min(p,Math.max(0,+$('#down').value||0)),l=p-d,r=Math.max(0,+$('#rate').value||0)/1200,m=Math.max(1,+$('#years').value||1)*12;
 const e=l===0?0:r===0?l/m:l*r*Math.pow(1+r,m)/(Math.pow(1+r,m)-1),total=e*m;
 $('#emiv').textContent=money(e);$('#loanv').textContent=money(l);$('#intv').textContent=money(Math.max(0,total-l));$('#payv').textContent=money(total);
}
...
$('#form')?.addEventListener('submit',e=>{
 ...
 const text=`*NELLAI ASSETS*\n━━━━━━━━━━━━━━━━━━━━\n*PROPERTY ENQUIRY*\n\n*CUSTOMER DETAILS*\nName          : ${d.get('name')}\nMobile        : ${d.get('phone')}\nPreferred Area: ${d.get('area')||'Not specified'}\nBudget        : ${d.get('budget')||'Not specified'}\n\n*PROPERTY REQUIREMENT*\nProperty      : ${d.get('property')}\nPreferred     : ${d.get('contact')}\nRequirement   : ${d.get('message')||'Not provided'}\n\n━━━━━━━━━━━━━━━━━━━━\nNellai Assets\nTirunelveli Property & Real Estate`;
 window.open('https://wa.me/919360390690?text='+encodeURIComponent(text),'_blank');$('#msg').textContent='Enquiry prepared in WhatsApp.';
});